...
$GitHubApiBase = httpsapi.github.com
$TempDir = Join-Path $envTEMP GitHubScriptLauncher
$AppVersion = 3.0
$AppDate = 2026-02-18
...
public class DarkListView  ListView
{
    ...
    public Dictionarystring, Color BadgeColors = new Dictionarystring, Color();
...
function Get-RuntimeStatus {
    ...
    $gitBash = CProgram FilesGitbinbash.exe
...
function Get-GitHubRepos {
    try {
        $h = @{ User-Agent = GitHubScriptLauncher3.0 }
        return (Invoke-RestMethod -Uri $GitHubApiBaseusers$GitHubUserreposper_page=100&sort=updated -Headers $h -ErrorAction Stop)
    } catch { return $null }
}
...
function Get-RepoContents {
    ...
    try {
        $h = @{ User-Agent = GitHubScriptLauncher3.0 }
        return (Invoke-RestMethod -Uri $GitHubApiBaserepos$GitHubUser$RepoNamecontents$Path -Headers $h -ErrorAction Stop)
    } catch { return $null }
}
...
$loadRepos = {
    ...
    foreach ($repo in $repos) {
        $scriptrepoData[$repo.name] = $repo
        $scriptallRepoNames += $repo.name
...
$txtRepoSearch.Add_TextChanged({
    ...
    $lstRepos.Items.Clear()
    foreach ($name in $scriptallRepoNames) {
...
$lstRepos.Add_SelectedIndexChanged({
    ...
    if ($null -ne $repo) {
        ...
        try {
            $dt = [datetime]Parse($repo.updated_at)
            $stats += Aktualisiert $($dt.ToString('dd.MM.yyyy HHmm'))
        } catch { }
    ...
    if ($scriptruntimes.Count -eq 0) { $scriptruntimes = Get-RuntimeStatus }

    $scripts = Get-ScriptFilesRecursive -RepoName $repoName
    $scriptscriptData = $scripts
    ...
    if ($scripts.Count -eq 0) {
        ...
        foreach ($s in $scripts) {
            $rt = $s.Runtime
            $rtInfo = $null
            if ($scriptruntimes.ContainsKey($rt)) { $rtInfo = $scriptruntimes[$rt] }
...
$lvScripts.Add_SelectedIndexChanged({
    ...
    $scriptselectedScript = $lvScripts.SelectedItems[0].Tag
    $btnRun.Enabled = $true
    $btnSave.Enabled = $true
    $lblPreviewFile.Text = $scriptselectedScript.Path
    $statusLabel.Text = Lade Vorschau...
    $form.Cursor = [System.Windows.Forms.Cursors]WaitCursor
    try {
        $content = (Invoke-WebRequest -Uri $scriptselectedScript.DownloadUrl -UseBasicParsing).Content
        $txtPreview.Text = $content
    } catch {
        $txtPreview.Text = Fehler $($_.Exception.Message)
    }
    $form.Cursor = [System.Windows.Forms.Cursors]Default
    $statusLabel.Text = Vorschau $($scriptselectedScript.Path)
})

$btnRun.Add_Click({
    if ($null -eq $scriptselectedScript) { return }
    $s = $scriptselectedScript
    $rt = $s.Runtime
    ...
    if ($rt -eq Python) {
        $pyCmd = $null
        if ($scriptruntimes.ContainsKey(Python) -and $null -ne $scriptruntimes[Python]) {
            $pyCmd = $scriptruntimes[Python].Command
        }
...
$btnOpenGH.Add_Click({
    if ($null -eq $lstRepos.SelectedItem) { return }
    $repoName = $lstRepos.SelectedItem.ToString()
    Start-Process httpsgithub.com$GitHubUser$repoName
})

$lblUser.Add_Click({
    Start-Process httpsgithub.com$GitHubUser
})

$btnRuntime.Add_Click({
    $scriptruntimes = Get-RuntimeStatus
    $lines = @()
    $lines +=   === Runtime-Check ===
    $lines += 
    foreach ($r in $scriptruntimes.GetEnumerator()  Sort-Object Name) {
...
# START
# ============================================================
$form.Add_Shown({ $scriptruntimes = Get-RuntimeStatus; & $loadRepos })
